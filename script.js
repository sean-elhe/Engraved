...
async function loadChapters() {
    const response = await fetch(
        `/api/chapters?translation=${selectedTranslation}&book=${getBookName()}`
    );
...
async function loadChapter() {
    ...
    const response = await fetch(
        `/api/chapter?translation=${selectedTranslation}` +
        `&book=${getBookName()}` +
        `&chapter=${getChapter()}`
    );
...
async function loadSavedChaptersUI() {
...
async function initializeApp() {
    ...
    openBtn2.addEventListener("click", () => overlay2.classList.remove("hidden"));

    closeBtn.addEventListener("click", () => overlay.classList.add("hidden"));
    closeBtn2.addEventListener("click", () => overlay2.classList.add("hidden"));
    ...
    overlay2.addEventListener("click", e => {
        if (e.target === overlay2) overlay2.classList.add("hidden");
    });
